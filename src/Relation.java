...
public class Relation {
	...
	public int readFromBuffer(Record record, ByteBuffer buff, int pos) {
	    try {
	        record.getValeursRec().clear(); // Vider les anciennes valeurs du record
	        buff.position(pos); // Positionner le tampon à la position spécifiée
	        int totalSize = 0; // Variable pour calculer la taille totale
	        ...
	        for (int i = 0; i < getNbCol(); i++) {
	            ...
	            switch (colInfo.getTypeCol()) {
	                ...
	                case FLOAT:
	                    // Lire un FLOAT (4 octets)
	                    float valeur_float = buff.getFloat(); 
	                    record.getValeursRec().add(Float.toString(valeur_float)); // Ajoute le float au record sous forme de chaîne
	                    totalSize += 4; // Taille d'un FLOAT est de 4 octets
	                    break;

	                case CHAR:
	                    // Lire un CHAR : longueur fixe
	                    int charLength = colInfo.getLengthChar();
	                    byte[] charBytes = new byte[charLength];
	                    buff.get(charBytes); // Lire les octets de la chaîne CHAR
	                    String valeur_char = new String(charBytes).trim(); // Retirer les espaces supplémentaires
	                    record.getValeursRec().add(valeur_char);
	                    totalSize += charLength; // Taille du CHAR
	                    break;

	                case VARCHAR:
	                    // Lire un VARCHAR : 4 octets pour la longueur + chaîne
	                    ...
	                    totalSize += 4 + varCharLength; // Taille de VARCHAR = 4 (longueur) + contenu
	                    break;

	                default:
	                    throw new IllegalArgumentException("Le type de votre colonne est invalide !");
	            */
	            case INT:
                    int intValue = buff.getInt(); 
                    record.getValeursRec().add(Integer.toString(intValue));
                    totalSize += 4;
                    break;
                case FLOAT:
                    float floatValue = buff.getFloat();
                    record.getValeursRec().add(Float.toString(floatValue));
                    totalSize += 4;
                    break;
                case CHAR:
                    int stringLength = colInfo.getLengthChar();
                    byte[] stringBytes = new byte[stringLength];
                    buff.get(stringBytes);
                    String stringValue = new String(stringBytes).trim(); // remove spaces or extra-padding
                    record.getValeursRec().add(stringValue);
                ...
                case VARCHAR:
                    ...
                    record.getValeursRec().add(varStringValue);
	        ...
	        System.out.println("Valeurs du record : " + record.getValeursRec());
	        System.out.println("la taille totale des données lues " + totalSize);
	        return totalSize; // Retourner la taille totale des données lues

	    } catch (Exception e) {
	        System.err.println("Erreur dans readFromBuffer: " + e.getMessage());
